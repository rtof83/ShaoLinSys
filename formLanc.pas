...
begin
 if dtmdados.qrycaixaTipo.asstring = 'SAÍDA' then
...
begin
 ...
 if (dataini <> '') and (datafin <> '') then
  dtmdados.qryLanc.SQL.Text := 'SELECT DISTINCT((select sum(valor) from caixa where tipo = ''ENTRADA'' and pagamento >= ' + dataini + ' and pagamento <= ' + datafin + ') - ' +
                               '(select sum(valor) from caixa where tipo = ''SAÍDA'' and pagamento >= '   + dataini + ' and pagamento <= ' + datafin + ')) as TotalGeral, '     +
                               '(select sum(valor) from caixa where tipo = ''ENTRADA'' and pagamento >= ' + dataini + ' and pagamento <= ' + datafin + ') as Entrada, '       +
                               '(select sum(valor) from caixa where tipo = ''SAÍDA'' and pagamento >= '   + dataini + ' and pagamento <= ' + datafin + ') as Saida FROM Caixa'
 else
  dtmdados.qryLanc.SQL.Text := 'SELECT DISTINCT((select sum(valor) from caixa where tipo = ''ENTRADA'') - ' +
                              '(select sum(valor) from caixa where tipo = ''SAÍDA'')) as TotalGeral, '      +
                              '(select sum(valor) from caixa where tipo = ''ENTRADA'') as Entrada, '        +
                              '(select sum(valor) from caixa where tipo = ''SAÍDA'') as Saida FROM Caixa';
...
begin
 ...
 dtmdados.qryLanc.SQL.Clear;
 if (dataini <> '') and (datafin <> '') then
  dtmdados.qryLanc.SQL.Text := 'SELECT SUM(valor) as TotalGeral, (select sum(valor) from caixa where tipo = ''SAÍDA'' and datacad >= ' + dataini + ' and datapag <= ' + datafin + ') as Saida, ' +
                               '(sum(valor) - sum(valor)) as Entrada FROM Caixa WHERE tipo = ''SAÍDA'' and datacad >= ' + dataini + ' and datapag <= ' + datafin
 else
  dtmdados.qryLanc.SQL.Text := 'SELECT SUM(valor) as TotalGeral, (select sum(valor) from caixa where tipo = ''SAÍDA'') as Saida, ' +
                               '(sum(valor) - sum(valor)) as Entrada FROM Caixa WHERE tipo = ''SAÍDA''';
 dtmdados.qryLanc.Open;

 dtmdados.qryCaixa.Filtered := false;
 if (dataini <> '') and (datafin <> '') then
  dtmdados.qryCaixa.Filter   := 'tipo = ''SAÍDA'' and datacad >= ' + dataini + ' and datapag <= ' + datafin
 else
  dtmdados.qryCaixa.Filter   := 'tipo = ''SAÍDA''';
 dtmdados.qryCaixa.Filtered := true;
end;
...
begin
 ...
 dtmdados.qryLanc.SQL.Text := 'SELECT DISTINCT((select sum(valor) from caixa where tipo = ''ENTRADA'' and pendente = ''X'' and datapag < date()) - ' +
                              '(select sum(valor) from caixa where tipo = ''SAÍDA'' and pendente = ''X'' and datapag < date())) as TotalGeral, '     +
                              '(select sum(valor) from caixa where tipo = ''ENTRADA'' and pendente = ''X'' and datapag < date()) as Entrada, '       +
                              '(select sum(valor) from caixa where tipo = ''SAÍDA'' and pendente = ''X'' and datapag < date()) as Saida FROM Caixa';
 dtmdados.qryLanc.Open;
...
begin
 ...
 dtmdados.qryLanc.SQL.Clear;
 if (dataini <> '') and (datafin <> '') then
  dtmdados.qryLanc.SQL.Text := 'SELECT DISTINCT(select sum(valor) from caixa where tipo = ''SAÍDA'' and pendente = ''X'' and datacad >= ' + dataini + ' and datapag <= ' + datafin + ') as TotalGeral, ' +
                               '(select sum(valor) from caixa where tipo = ''SAÍDA'' and pendente = ''X'' and datacad >= ' + dataini + ' and datapag <= ' + datafin + ') as Saida, '                     +
                               '(sum(valor) - sum(valor)) as Entrada FROM Caixa'
 else
  dtmdados.qryLanc.SQL.Text := 'SELECT DISTINCT(select sum(valor) from caixa where tipo = ''SAÍDA'' and pendente = ''X'') as TotalGeral, ' +
                               '(select sum(valor) from caixa where tipo = ''SAÍDA'' and pendente = ''X'') as Saida, '                     +
                               '(sum(valor) - sum(valor)) as Entrada FROM Caixa';
 dtmdados.qryLanc.Open;

 dtmdados.qryCaixa.Filtered := false;
 if (dataini <> '') and (datafin <> '') then
  dtmdados.qryCaixa.Filter   := 'tipo = ''SAÍDA'' and pendente = ''X'' and datacad >= ' + dataini + ' and datapag <= ' + datafin
 else
  dtmdados.qryCaixa.Filter   := 'tipo = ''SAÍDA'' and pendente = ''X''';
 dtmdados.qryCaixa.Filtered := true;
end;
...
begin
 ...
 if (dataini <> '') and (datafin <> '') then
  dtmdados.qryLanc.SQL.Text := 'SELECT DISTINCT((select sum(valor) from caixa where tipo = ''ENTRADA'' and pendente is null and pagamento >= ' + dataini + ' and pagamento <= ' + datafin + ') - ' +
                               '(select sum(valor) from caixa where tipo = ''SAÍDA'' and pendente is null and pagamento >= ' + dataini + ' and pagamento <= ' + datafin + ')) as TotalGeral, '     +
                               ...
                               '(select sum(valor) from caixa where tipo = ''SAÍDA'' and pendente is null and pagamento >= ' + dataini + ' and pagamento <= ' + datafin + ') as Saida FROM Caixa'
 else
  dtmdados.qryLanc.SQL.Text := 'SELECT DISTINCT((select sum(valor) from caixa where tipo = ''ENTRADA'' and pendente is null) - ' +
                               '(select sum(valor) from caixa where tipo = ''SAÍDA'' and pendente is null)) as TotalGeral, '     +
                               '(select sum(valor) from caixa where tipo = ''ENTRADA'' and pendente is null) as Entrada, '       +
                               '(select sum(valor) from caixa where tipo = ''SAÍDA'' and pendente is null) as Saida FROM Caixa';
 dtmdados.qryLanc.Open;
...
begin
 ...
 if btngeral.Down     = true then frmprincipal.rvpShaoLin.SetParam('tipo', 'Lançamento Geral' + periodo);
 if btnentrada.Down   = true then frmprincipal.rvpShaoLin.SetParam('tipo', 'Entradas' + periodo);
 if btnsaida.Down     = true then frmprincipal.rvpShaoLin.SetParam('tipo', 'Saídas' + periodo);
 if btnpendencia.Down = true then frmprincipal.rvpShaoLin.SetParam('tipo', 'Pendências' + periodo);
